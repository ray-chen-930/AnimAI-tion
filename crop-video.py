...
graph = tf.Graph()
return_tensors = utils.read_pb_return_tensors(graph, yolo_pb, return_elements)
...
def save_bbox_to_file(start, end, fps, tube_bbox, frame_shape, cur_traj, inp, image_shape,
    ...
    if pan_thresh:
        ...
        end_bbox = extract_bbox(end_frame, fa)
        start_bbox = extract_bbox(start_frame, fa)
        print('          tube bbox', tube_bbox)
        print('          start box', start_bbox, len(start_bbox))
        print('          end box', end_bbox, len(end_bbox))
...
if __name__ == "__main__":
    ...
    # cmd line run
    # CUDA_VISIBLE_DEVICES=0 python3 crop-video-testing.py --inp data/kon_2.mkv --min_frames 60 --out /sizigi/coruscant/pond/data_proc/fom_ray/v3_addpan_low_res/kon2/train --name kon2 --pan_thresh .25
